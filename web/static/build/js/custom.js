...
function init_flot_chart() {
    ...
    $.getJSON("http://192.168.56.1:4080/api/root/exerciseLogs/", function(data) {
        var exerciseGraphData = []
        $.each(data, function(i, field) {
            var date = field.pub_date
            var year = date.substr(0,4)
            var month = date.substr(5,2)
            var day = date.substr(8,2)

            var tmp = [gd(year, month, day), field.counts]
            exerciseGraphData.push(tmp)
        })
        if ($("#chart_plot_01").length) {
            $.plot($("#chart_plot_01"), [exerciseGraphData], chart_plot_01_settings);
        }
    })
        ...
        var lineChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    pointBorderWidth: 1,
                    data: [61, 62, 59, 63, 62.5, 63, 64, 63.2, 61]
                }]
            },
        });
...
function init_survey() {
    ...
    $("input[name=headache_radio]").change(function() {
        var value = $(this).val();
        if(value == "y") {
...
function init_SmartWizard() {
    ...
    $('#wizard').smartWizard({
        ...
        labelNext:'다음 >', // label for Next button
        labelPrevious:'< 이전', // label for Previous button
        labelFinish:'완료',  // label for Finish button        
